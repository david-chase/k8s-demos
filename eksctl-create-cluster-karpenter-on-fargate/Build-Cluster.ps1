cls
Write-Host ""
Write-Host ::: Build EKS Cluster With Karpenter on Fargate v1 ::: -ForegroundColor Cyan
Write-Host ""
...
Write-Host 'helm upgrade --install karpenter oci://public.ecr.aws/karpenter/karpenter --version "$env:KARPENTER_VERSION" `
  ...
  --set serviceAccount.annotations."eks\.amazonaws\.com/role-arn"="arn:'$env:AWS_PARTITION':iam::'$env:AWS_ACCOUNT_ID':role/'$env:CLUSTER_NAME'-karpenter" `
  --wait' -ForegroundColor Green
helm registry logout public.ecr.aws
$sAnnotation = "arn:" + $env:AWS_PARTITION + ":iam::" + $env:AWS_ACCOUNT_ID + ":role/" + $env:CLUSTER_NAME + "-karpenter"
helm upgrade --install karpenter oci://public.ecr.aws/karpenter/karpenter --version "$env:KARPENTER_VERSION" --namespace "$env:KARPENTER_NAMESPACE" --create-namespace `
  ...
  --set controller.resources.limits.cpu=1 `
  --set controller.resources.limits.memory=1Gi `
  --set serviceAccount.annotations."eks\.amazonaws\.com/role-arn"=$sAnnotation `
  --wait
